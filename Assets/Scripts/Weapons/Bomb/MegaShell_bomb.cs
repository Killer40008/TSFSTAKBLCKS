        ...
        while (rigit != null)
        {
            ...
            if (rigit.velocity.normalized.y <= 0)
            {
                ...
                for (int i = 0; i < 12; i++)
                {
                    ...
                    wObj.tag = "Bomb";
                    WeaponsCombo.CurrentWeapon = wObj.AddComponent<Large_Bomb>();
                    Sprite[] sprites = Managers.SpawnManager.BombSprites;
                    Object[] Explosions = Managers.SpawnManager.BombExplosions;
                    WeaponsCombo.CurrentWeapon.Create(sprites[1], Explosions[0], Bomb.Strength, Bomb.SoruceTank);
